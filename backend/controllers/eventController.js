...
const buildDateTime = (dateVal, timeStr) => {
  let y, m, d;
  if (!dateVal) {
    const now = new Date();
  ...
  const hour = timeComp ? timeComp.hour : 0;
  const min = timeComp ? timeComp.minute : 0;
  return new Date(y, m, d, hour, min, 0, 0);
};
...
const recalculateTaskSchedule = (event) => {
  const eventStart = buildDateTime(event.eventDate, event.startTime);
  let currentPointer = new Date(eventStart.getTime());
  ...
  for (const task of event.tasks) {
    ...
    const isVerifiedAndCompleted = task.status === "VERIFIED" && Number(task.progress) === 100;

    if (isVerifiedAndCompleted && task.verifiedAt) {
      const actualVerifiedTime = new Date(task.verifiedAt);
      if (!isNaN(actualVerifiedTime.getTime())) {
        currentPointer = new Date(actualVerifiedTime.getTime());
      }
    } else {
      task.startTime = formatTimeFromDate(currentPointer);
      const taskEnd = new Date(currentPointer.getTime() + duration * 60 * 1000);
      task.endTime = formatTimeFromDate(taskEnd);
      currentPointer = taskEnd;
    }
  }

  const effectiveEndDate = event.endDate || event.eventDate;
  const eventEnd = buildDateTime(effectiveEndDate, event.endTime);

  if (currentPointer > eventEnd) {
    event.endTime = formatTimeFromDate(currentPointer);
    event.endDate = new Date(currentPointer.getFullYear(), currentPointer.getMonth(), currentPointer.getDate());
  }

  const updatedEventEnd = buildDateTime(event.endDate || event.eventDate, event.endTime);
  const totalEventMinutes = Math.round((updatedEventEnd.getTime() - eventStart.getTime()) / (1000 * 60));
  const remainingMinutes = totalEventMinutes - totalTaskMinutes;
...
const syncEventStatuses = async () => {
  try {
    ...
    for (const event of events) {
      ...
      const eventStart = buildDateTime(event.eventDate, event.startTime);
      const eventEnd = buildDateTime(event.endDate || event.eventDate, event.endTime);
      ...
      if (event.status !== "COMPLETED") {
        ...
        for (const task of event.tasks) {
          if (task.status === "PENDING" || task.status === "IN_PROGRESS") {
            const taskEnd = buildDateTime(event.eventDate, task.endTime);
            if (taskEnd && now > taskEnd) {
              task.status = "DELAYED";
              eventModified = true;
...
const createEvent = async (req, res) => {
  try {
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message: "Number of coordinators must be at least 1",
      });
    }
    ...
    const eventStart = buildDateTime(eventDate, startTime);
    const eventEnd = buildDateTime(finalEndDate, endTime);
    ...
    for (let index = 0; index < tasks.length; index++) {
      ...
      if (
        ...
        return res.status(400).json({
          success: false,
          message: `Task ${index + 1} must have a valid taskName`,
        });
      }
      ...
      if (!Number.isFinite(duration) || duration <= 0) {
        return res.status(400).json({
          success: false,
          message: `Task ${index + 1} (${task.taskName}) must have a valid duration in minutes`,
        });
      }
      ...
      if (!Number.isInteger(duration)) {
        return res.status(400).json({
          success: false,
          message: `Task ${index + 1} (${task.taskName}) duration must be in whole minutes`,
        });
      }
      ...
      formattedTasks.push({
        ...
        guidelines: typeof task.guidelines === "string" ? task.guidelines.trim() : "",
        remarks: typeof task.remarks === "string" ? task.remarks.trim() : "",
    ...
    const event = new Event({
      ...
      location: location || { address: eventPlace.trim() },
      tasks: formattedTasks,
      organizerId: req.user.id,
      status: "DRAFT",
    });

    const schedule = recalculateTaskSchedule(event);
    await event.save();
...
const updateTask = async (req, res) => {
  try {
    ...
    if (progress !== undefined) {
      ...
      if (task.status === "VERIFIED" && task.progress === 100 && !task.verifiedAt) {
...
const updateEvent = async (req, res) => {
  try {
    ...
    if (endDate !== undefined) {
      event.endDate = endDate ? new Date(endDate) : new Date(event.eventDate);
    }
    ...
    if (numberOfCoordinators !== undefined) {
      ...
      event.numberOfCoordinators = Number(numberOfCoordinators);
    }

    if (eventPlace !== undefined) {
      event.eventPlace = eventPlace.trim();
    }
    ...
    if (tasks !== undefined) {
      ...
      for (let index = 0; index < tasks.length; index++) {
        ...
        if (!Number.isInteger(duration) || duration <= 0) {
          return res.status(400).json({
            success: false,
            message: `Task ${index + 1} must have a valid duration in minutes`,
          });
        }
        ...
        if (
          ...
          const isJoinedUser = event.coordinators.some(
            (userId) => userId.toString() === task.coordinator.toString()
          );
