...
export const OrganizerRegister = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await register('ORGANIZER', {
      ...
      if (res.success) {
        navigate(`/verify-email?email=${encodeURIComponent(formData.email.trim())}&role=ORGANIZER`);
      } else {
        setErrorMsg(res.message || 'Organizer registration failed.');
      }
    } catch (err) {
      const msg = err.response?.data?.message || 'Server error during organizer registration.';
      setErrorMsg(msg);
    }
  };
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg-dark)', padding: '24px' }}>
      <div className="form-card animate-fade-in" style={{ width: '100%', maxWidth: '520px' }}>
        ...
        <h1 style={{ fontSize: '1.75rem', fontWeight: 800, marginBottom: '6px' }}>Organizer Registration</h1>
        <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', marginBottom: '24px' }}>
          Register your organization to host & publish events.
        </p>
        ...
        <form onSubmit={handleSubmit} autoComplete="off">
          ...
          <button type="submit" className="gradient-btn" style={{ width: '100%', padding: '12px' }} disabled={loading}>
            {loading ? 'Registering Organization...' : 'Register Organizer'}
          </button>
        </form>

        <p style={{ marginTop: '20px', textAlign: 'center', fontSize: '0.875rem', color: 'var(--text-muted)' }}>
          Registering as a user instead?{' '}
          <Link to="/register/user" style={{ color: 'var(--primary)', fontWeight: '700' }}>
            Register as User
          </Link>
        </p>
      </div>
